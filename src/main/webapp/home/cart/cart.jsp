	...
	<table>
	    ...
	    <c:set var="counter" value="1" /> <!-- 번호 카운터 초기화 -->
	    <c:set var="totalAmount" value="0" /> <!-- 총액 초기화 -->
	    <c:forEach var="cart" items="${cartList}">
	        <c:set var="rowTotal" value="${cart.price * cart.quantity}" /> <!-- 각 행의 총액 계산 -->
	        <c:set var="totalAmount" value="${totalAmount + rowTotal}" /> <!-- 전체 총액 누적 -->
	        <tr>
	            <td>${counter}</td> <!-- 카운터 값 출력 -->
	            ...
	            <td><fmt:formatNumber value="${rowTotal}" groupingUsed="true"/>원</td> <!-- 행별 총액 표시 -->
	            <td>
	                <form action="/cart/modify.html" method="post" onsubmit="return confirmDelete(this);">
	        ...
	        <c:set var="counter" value="${counter + 1}" /> <!-- 카운터 증가 -->
	    </c:forEach>
	</table>
		<span style="font-weight: bold; color: red;">
		<p>총 액 : <fmt:formatNumber value="${totalAmount}" groupingUsed="true"/>원</p> <!-- 최종 총액 표시 -->
		</span>
		...
		<button type="button" class="cart-button" onclick="gohome()">상품 보러 가기</button>
	</div>
	<script type="text/javascript">
	function gohome() {
